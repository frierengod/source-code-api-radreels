<?php
/**
 * Contoh Script Bridge API Radreels via StreamAPI.web.id
 * ----------------------------------------------------
...
// Fungsi bantuan buat manggil endpoint Radreels di sistem Bridge
function callRadreelsAPI($urlTujuan, $token) {
...
// UJI COBA KODE
// -----------------------------------------------------------------
$baseUrl = "https://streamapi.web.id/p/radreels/api/v1";

// Contoh 1: Fitur Cari Film Radreels
echo "\n=====================[ 1. Pencarian Radreels ]==================\n";
$kataKunci = urlencode("cinta");
$halaman = 1;
$urlCari = "{$baseUrl}/search/{$kataKunci}/{$halaman}?lang=in";

$hasilCari = callRadreelsAPI($urlCari, $API_TOKEN);
if ($hasilCari) {
...
$hasilDetail = callRadreelsAPI($urlDetail, $API_TOKEN);
if ($hasilDetail) {
...
$hasilNonton = callRadreelsAPI($urlNonton, $API_TOKEN);
if ($hasilNonton) {
...
echo "▶▶▶ https://streamapi.web.id/docs/radreels ◀◀◀\n\n";
